...
def register_user(data: schemas.UsuarioCreate, db: Session = Depends(get_db)):
    ...
    # Gera token de verificação e token de navegação
...
def login(data: schemas.Login, db: Session = Depends(get_db)):
    ...
    # Garante que o usuário tenha um token fixo de navegação
...
# =====================================================
# ROTA: VERIFICAR E-MAIL (link do primeiro acesso)
# =====================================================
...
def verify_email(
    ...
    if not getattr(usuario, "verification_token", None):
        raise HTTPException(
            status_code=400, detail="Conta já verificada ou token inválido."
        )
